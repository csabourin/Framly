...
const RightPanel: React.FC = () => {
  ...
  return (
    ...
    >
      {/* Tab Bar */}
      <div className="flex border-b border-gray-200/60 dark:border-gray-700/60 bg-gray-50/50 dark:bg-gray-800/50">
        {tabs.map((tab) => {
          const Icon = tab.icon;
      ...
      <div className="flex-1 overflow-y-auto relative">
        <div style={{ display: rightPanelTab === 'properties' ? 'block' : 'none' }}>
          <PropertiesPanel />
        </div>
        <div style={{ display: rightPanelTab === 'components' ? 'block' : 'none' }}>
          <ComponentPanel />
        </div>
      </div>
    </aside>
