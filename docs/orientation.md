...
| **`:empty` never matches a canvas element** | The clickable floor for elements that render nothing was first written as `.canvas-element:empty`. It matched nothing in any state: the wrapper always contains the `--element-*` carrier div, and a selected element gains a `.selection-handle` beside it. `CanvasElement` now sets `data-renders-nothing`, and the rule keys off that. The test asserts the computed `min-width`, not the rendered box — a section is over 32px on its own, so measuring the box passes with the floor switched off. |
